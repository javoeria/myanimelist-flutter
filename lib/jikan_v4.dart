...
class JikanV4 {
  Future<List<dynamic>> getGenreList({bool anime = true}) async {
    ...
    final response = await http.get(Uri.parse('$apiBaseUrl/genres/$type?order_by=name&sort=asc'));
  ...
  Future<BuiltList<Map<String, dynamic>>> getProducerList({bool anime = true, int page = 1}) async {
    ...
    final response = await http.get(Uri.parse('$apiBaseUrl/$type?order_by=count&sort=desc&page=$page'));
  ...
  Future<List<dynamic>> getVideos(String type, String subtype) async {
    final response = await http.get(Uri.parse('$apiBaseUrl/watch/$type/$subtype'));
  ...
  Future<List<dynamic>> getReviews({bool anime = true}) async {
    final type = anime ? 'anime' : 'manga';
    final response = await http.get(Uri.parse('$apiBaseUrl/reviews/$type'));
  ...
  Future<List<dynamic>> getRecommendations({bool anime = true}) async {
    final type = anime ? 'anime' : 'manga';
    final response = await http.get(Uri.parse('$apiBaseUrl/recommendations/$type'));
